...
output: 
  rmarkdown::html_vignette:
    fig_caption: yes
vignette: >
  %\VignetteIndexEntry{Using the GSPE Shiny Package}
...
You will also need to install the `FPBKPack2` package from GitHub. The easiest way to do this is to use the `install_git` function in the `devtools` library. You will need to first install `devtools` if you have not installed this package on your machine previously.
...
## On a Mac, the ~ starts the path on your home directory
shiny::runGitHub("Shiny-FPBK", "highamm", 
  destdir = "~/Desktop/appfolder",
  launch.browser = TRUE)

## full path name for a PC
shiny::runGitHub("Shiny-FPBK", "highamm",
  destdir = "C:/Users/highamm/Desktop/appfolder",
  launch.browser = TRUE)

## full path name for a Mac
shiny::runGitHub("Shiny-FPBK", "highamm",
  destdir = "/Users/highamm/Desktop/appfolder",
  launch.browser = TRUE)
```
...
In order to use this feature, first make sure that `rgeos`, `maptools`, `rgdal`, `sp`, and `broom` packages are all installed. This only need to be installed once.
